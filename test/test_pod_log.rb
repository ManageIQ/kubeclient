...
class TestPodLog < MiniTest::Test
  ...
  def test_get_pod_log_since_time
    ...
    client = Kubeclient::Client.new('http://localhost:8080/api/', 'v1')
    retrieved_log = client.get_pod_log(
      'redis-master-pod', 'default',
      timestamps: true, since_time: '2018-04-27T18:30:17.480321984Z'
    )
  ...
  def test_get_pod_log_tail_lines
    ...
    stub_request(:get, %r{/namespaces/default/pods/[a-z0-9-]+/log})
      .to_return(body: selected_lines, status: 200)

    client = Kubeclient::Client.new('http://localhost:8080/api/', 'v1')
    retrieved_log = client.get_pod_log('redis-master-pod', 'default', tail_lines: 2)
  ...
  def test_get_pod_limit_bytes
    ...
    stub_request(:get, %r{/namespaces/default/pods/[a-z0-9-]+/log})
      .to_return(body: selected_bytes, status: 200)

    client = Kubeclient::Client.new('http://localhost:8080/api/', 'v1')
    retrieved_log = client.get_pod_log('redis-master-pod', 'default', limit_bytes: 10)
